...
export function injectHubClientBridge() {
  const bridge = {
    ...
    saveAuthToken: (token: string) => invoke("save_auth_token", { token }),
    ...
    getAvailableDevices: () => invoke("get_available_devices"),
    selectDirectDevice: (device: unknown) => invoke("select_direct_device", { device }),
