...
// WebKitGTK before 2.22 has no jsc_value API (webkit_javascript_result_get_js_value /
// jsc_value_to_string). Convert IPC payloads through the JavaScriptCore C API instead,
// which exists in every webkit2gtk-4.0 build, so the binary keeps working on
// webkit2gtk 2.20 (Kylin V10 / Ubuntu 16.04 era).
mod legacy_jsc {
  use std::os::raw::{c_char, c_void};
  extern "C" {
    pub fn webkit_javascript_result_get_global_context(result: *mut c_void) -> *mut c_void;
    pub fn webkit_javascript_result_get_value(result: *mut c_void) -> *mut c_void;
    pub fn JSValueToStringCopy(
      ctx: *mut c_void,
      value: *mut c_void,
      exception: *mut *mut c_void,
    ) -> *mut c_void;
    pub fn JSStringGetMaximumUTF8CStringSize(string: *mut c_void) -> usize;
    pub fn JSStringGetUTF8CString(string: *mut c_void, buffer: *mut c_char, size: usize) -> usize;
    pub fn JSStringRelease(string: *mut c_void);
  }
}

fn javascript_result_to_string(msg: &webkit2gtk::JavascriptResult) -> Option<String> {
  use glib::translate::ToGlibPtr;
  unsafe {
    let raw: *mut webkit2gtk_sys::WebKitJavascriptResult = msg.to_glib_none().0;
    let raw = raw as *mut std::os::raw::c_void;
    let ctx = legacy_jsc::webkit_javascript_result_get_global_context(raw);
    let value = legacy_jsc::webkit_javascript_result_get_value(raw);
    if ctx.is_null() || value.is_null() {
      return None;
    }
    let js_string = legacy_jsc::JSValueToStringCopy(ctx, value, std::ptr::null_mut());
    if js_string.is_null() {
      return None;
    }
    let capacity = legacy_jsc::JSStringGetMaximumUTF8CStringSize(js_string);
    let mut buffer = vec![0u8; capacity.max(1)];
    let written =
      legacy_jsc::JSStringGetUTF8CString(js_string, buffer.as_mut_ptr() as *mut _, buffer.len());
    legacy_jsc::JSStringRelease(js_string);
    // written includes the trailing NUL byte
...
impl InnerWebView {
  pub fn new(
    ...
    manager.connect_script_message_received(None, move |_m, msg| {
      ...
      if let Some(js) = javascript_result_to_string(msg) {
