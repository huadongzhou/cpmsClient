...
fn autostart_is_enabled(app: AppHandle) -> CommandResult<bool> {
    match autostart_manager(&app).and_then(|manager| manager.is_enabled().map_err(|error| error.to_string())) {
        Ok(value) => CommandResult::ok(value),
...
fn build_tray() -> SystemTray {
    ...
    SystemTray::new().with_menu(tray_menu).with_tooltip("CPMS Client")
}
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            setup_client_event_bridge(&app_handle);
