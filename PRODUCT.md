...
- 获取本地websocket端口  
  ...
  2.获取cpms客户端下的 DriverClient.ini 文件3.获取其中的 WebsocketPort 端口信息
- 在本地该端口监听 websocket 服务（客户端作为监听端，不主动连接）
- 等待连接接入并推送任务，监听推送的任务1.解析任务消息 拿到任务文件路径2.通过客户端的上传接口 携带iframe实例内的token 转发任务
...
选择机器包含服务端更新和客户端会话 deviceId 更新两步。
