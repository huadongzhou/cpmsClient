...
/** 确保通知子窗口存在（show 与诊断流程共用）。 */
export async function ensureNotificationWindow() {
...
/** 将通知窗口定位到屏幕右下角（show 与诊断流程共用）。 */
export async function positionNotificationWindow(notificationWindow: WebviewWindow) {
...
function isDuplicateWindowLabelError(error: unknown) {
  ...
  return /window.*label.*already exists/i.test(message);
}
